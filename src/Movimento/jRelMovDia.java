...
public class jRelMovDia extends javax.swing.JInternalFrame {
    Db conn = VariaveisGlobais.conexao;
    JRViewer visor;

    /** Creates new form jExtrato */
    public jRelMovDia() throws JRException {
        initComponents();
        
        // Icone da tela
        FlatSVGIcon icone = new FlatSVGIcon("menuIcons/MovimentoDiario.svg",16,16);
        setFrameIcon(icone);        
    ...
    private void Imprimir(String dia) {
        ...
        String sSql = "SELECT e.rgprp, e.rgimv, e.contrato, e.campo, e.dtvencimento, e.dtrecebimento, e.rc_aut, r.campo rcampo FROM extrato e, recibo r WHERE (e.contrato = r.contrato AND e.rgprp = r.rgprp AND e.rgimv = r.rgimv AND e.dtvencimento = r.dtvencimento AND e.rc_aut = r.autenticacao) AND e.dtrecebimento = '&1.' ORDER BY e.contrato, e.dtvencimento;";
        ...
        try {
            while (imResult.next()) {
                ...
                String rcampo = imResult.getString("rcampo");
                ...
                float rseguro = PegaSeguro(rcampo);
                if (rseguro > 0) fseg += rseguro;
                ...
                // Aqui pega AL + NT
                float vrrecibo = PegaAluguel(rcampo);
                // Aqui soma mu + ju + co + ep + sg -> AL + NT
                vrrecibo += fmul + fjur + fcor + fexp + fseg;
                // Format o valor do recibo
        ...
        try {
            ...
            String fileName = "reports/rMovDiario.jasper";
            ...
            String outFileName = pathName + "movdiario_" + Dates.DateFormata("ddMMyyyy", new Date()) + ".pdf";
    ...
    private float PegaSeguro(String tmpCampo) throws SQLException {        
        String[][] rCampos = FuncoesGlobais.treeArray(tmpCampo, true);
        float fTotCred = 0; 
        for (int j = 0; j<rCampos.length; j++) {
            boolean bRetc = FuncoesGlobais.IndexOf(rCampos[j], "RT") > -1;
            if ("SG".equals(rCampos[j][4])) {
                fTotCred = LerValor.StringToFloat(LerValor.FormatNumber(rCampos[j][2],2));
            }
        }
        return fTotCred;
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setIconifiable(true);
        setTitle(".:: Relatório de Movimentação Diário");
        setMaximumSize(new java.awt.Dimension(248, 72));
        setMinimumSize(new java.awt.Dimension(248, 72));
        setPreferredSize(new java.awt.Dimension(248, 72));
        setVisible(true);
