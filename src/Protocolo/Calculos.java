...
public class Calculos {
    ...
    public void Inicializa(String rgprp, String rgimv, String contrato) throws SQLException {
        ...
        this.rComissao = LerValor.StringToFloat(LerValor.FormatNumber(conn.ReadParameters("comisvr"), 2));

        if (rgprp.equals("") && rgimv.equals("") && contrato.equals("")) {
            // nada
        } else {
            if (conn.ReadFieldsTable(new String[] {"rgimv"}, "multa", "rgimv = '" + this.rgimv + "'") != null) {
                Object[][] regFields = conn.ReadFieldsTable(new String[] {"te_per", "te_vrm", "txexp", "tbrliq", "multa_res", "multa_com", "dia_mul", "dia_jur", "per_jur", "cor_tipo", "cor_lim_a", "cor_corr", "dia_cor", "comissao", "comisvr"}, "multa", "RGIMV = '" + this.rgimv + "'");
                if (!VariaveisGlobais.RegraCalculos) {
                    ...
                    this.rComissao = LerValor.StringToFloat(LerValor.FormatNumber(regFields[14][3].toString(), 2));
                } else {
                    float ttaxa_per = LerValor.StringToFloat(LerValor.FormatNumber(regFields[0][3].toString(), 3));
                    if (ttaxa_per != 0) this.taxa_per = ttaxa_per;
                    
                    float ttaxa_vrm = LerValor.StringToFloat(LerValor.FormatNumber(regFields[1][3].toString(), 2));
                    if (ttaxa_vrm != 0) this.taxa_vrm = ttaxa_vrm;
                    
                    String ttaxa_exp = regFields[2][3].toString();
                    if (!ttaxa_exp.trim().equals("0000000000")) this.taxa_exp = ttaxa_exp;
                    
                    //'this.taxa_brlq = regFields[3][3]
                    
                    float tmulta_res = LerValor.StringToFloat(LerValor.FormatNumber(regFields[4][3].toString(), 3));
                    if (tmulta_res !=0) this.multa_res = tmulta_res;
                    
                    float tmulta_com = LerValor.StringToFloat(LerValor.FormatNumber(regFields[5][3].toString(), 3));
                    if (tmulta_com != 0) this.multa_com = tmulta_com; 
                    
                    float tdia_mul = LerValor.StringToFloat(LerValor.FormatNumber(regFields[6][3].toString(), 0));
                    if (tdia_mul != 0) this.dia_mul = tdia_mul;
                    
                    int tdia_jur = Integer.valueOf(LerValor.FormatNumber(regFields[7][3].toString(), 0));
                    if (tdia_jur != 0) this.dia_jur = tdia_jur;
                    
                    float tper_jur = LerValor.StringToFloat(LerValor.FormatNumber(regFields[8][3].toString(), 3));
                    if (tper_jur != 0) this.per_jur = tper_jur;
                    
                    float tcor_tipo = LerValor.StringToFloat(regFields[9][3].toString());
                    if (tcor_tipo != 0) this.cor_tipo = tcor_tipo; 
                    
                    float tcor_lim_a = LerValor.StringToFloat(regFields[10][3].toString());
                    if (tcor_lim_a != 0) this.cor_lim_a = tcor_lim_a;
                    
                    float tcor_per = LerValor.StringToFloat(LerValor.FormatNumber(regFields[11][3].toString(), 3));
                    if (tcor_per != 0) this.cor_per = tcor_per; 
                    
                    float tcor_dia = LerValor.StringToFloat(LerValor.FormatNumber(regFields[12][3].toString(), 0));
                    if (tcor_dia != 0) this.cor_dia = tcor_dia;
                    
                    float tfComissao = LerValor.StringToFloat(LerValor.FormatNumber(regFields[13][3].toString(), 3));
                    if (tfComissao != 0) this.fComissao = tfComissao;
                    
                    float trComissao = LerValor.StringToFloat(LerValor.FormatNumber(regFields[14][3].toString(), 2));
                    if (trComissao != 0) this.rComissao = trComissao;
                }
            } else {
                if (conn.ReadFieldsTable(new String[] {"rgprp"}, "multa", "rgprp = '" + this.rgprp + "' AND IsNull(rgimv)") != null) {
                    Object[][] regFields = conn.ReadFieldsTable(new String[] {"te_per", "te_vrm", "txexp", "tbrliq", "multa_res", "multa_com", "dia_mul", "dia_jur", "per_jur", "cor_tipo", "cor_lim_a", "cor_corr", "dia_cor", "comissao", "comisvr"}, "multa", "RGPRP = '" + this.rgprp + "' AND IsNull(RGIMV)");
                    if (!VariaveisGlobais.RegraCalculos) {
                        this.taxa_per = LerValor.StringToFloat(LerValor.FormatNumber(regFields[0][3].toString(), 3));
                        this.taxa_vrm = LerValor.StringToFloat(LerValor.FormatNumber(regFields[1][3].toString(), 2));
                        this.taxa_exp = regFields[2][3].toString();
    ...
    public float Multa(String campos, String vecto, boolean padrao) throws SQLException {
        ...
        if (padrao) {
            ...
            multa_total += df_br_Campos - dc_lq_Campos;
    ...
    public String[] percADM(String mrgprp, String mrgimv) throws SQLException {
        ...
        if (conn.ReadFieldsTable(new String[] {"rgimv"}, "multa", "rgimv = '" + mrgimv + "'") != null) {
            regFields = conn.ReadFieldsTable(new String[] {"pa_multa", "pa_jur", "pa_cor", "txexp", "adm_multa", "adm_juros", "adm_correcao", "adm_expediente"}, "multa", "rgimv = '" + mrgimv + "'");
            if (regFields[4][3].toString().equalsIgnoreCase("0")) admmulta = LerValor.FormatNumber(regFields[0][3].toString(), 3);
            if (regFields[5][3].toString().equalsIgnoreCase("0")) admjuros = LerValor.FormatNumber(regFields[1][3].toString(), 3);
            if (regFields[6][3].toString().equalsIgnoreCase("0")) admcorrecao = LerValor.FormatNumber(regFields[2][3].toString(), 3);
            if (regFields[7][3].toString().equalsIgnoreCase("0")) admTxExp = LerValor.FormatNumber(regFields[3][3].toString(), 3);            
        } else {
            if (conn.ReadFieldsTable(new String[] {"rgprp"}, "multa", "rgprp = '" + mrgprp + "' AND IsNull(rgimv)") != null) {
                regFields = conn.ReadFieldsTable(new String[] {"pa_multa", "pa_jur", "pa_cor", "txexp", "adm_multa", "adm_juros", "adm_correcao", "adm_expediente"}, "multa", "rgprp = '" + mrgprp + "' AND IsNull(rgimv)");
                if (regFields[4][3].toString().equalsIgnoreCase("0")) admmulta = LerValor.FormatNumber(regFields[0][3].toString(), 3);
                if (regFields[5][3].toString().equalsIgnoreCase("0")) admjuros = LerValor.FormatNumber(regFields[1][3].toString(), 3);
                if (regFields[6][3].toString().equalsIgnoreCase("0")) admcorrecao = LerValor.FormatNumber(regFields[2][3].toString(), 3);
                if (regFields[7][3].toString().equalsIgnoreCase("0")) admTxExp = LerValor.FormatNumber(regFields[3][3].toString(), 3);
    ...
    public float[] percComissao(String rgprp, String rgimv) throws SQLException {
        ...
        if (conn.ReadFieldsTable(new String[] {"rgimv"}, "multa", "rgimv = '" + rgimv + "'") != null) {
            regFields = conn.ReadFieldsTable(new String[] {"comissao", "comisvr"}, "multa", "rgimv = '" + rgimv + "'");
            if (!VariaveisGlobais.RegraCalculos) {
                fComissao = Float.valueOf(LerValor.FormatNumber(regFields[0][3].toString(), 3).replace(",", "."));
                rComissao = Float.valueOf(LerValor.FormatNumber(regFields[1][3].toString(), 2).replace(",", "."));
            } else {
                float tfComissao = Float.valueOf(LerValor.FormatNumber(regFields[0][3].toString(), 3).replace(",", "."));
                if (tfComissao != 0) fComissao = tfComissao;
                
                float trComissao = Float.valueOf(LerValor.FormatNumber(regFields[1][3].toString(), 2).replace(",", "."));
                if (trComissao != 0) rComissao = trComissao;
            }
        } else {
            if (conn.ReadFieldsTable(new String[] {"rgprp"}, "multa", "rgprp = '" + rgprp + "' AND IsNull(rgimv)") != null) {
                regFields = conn.ReadFieldsTable(new String[] {"comissao", "comisvr"}, "multa", "rgprp = '" + rgprp + "' AND IsNull(rgimv)");
                if (!VariaveisGlobais.RegraCalculos) {
                    fComissao = Float.valueOf(LerValor.FormatNumber(regFields[0][3].toString(), 3).replace(",", "."));
                    rComissao = Float.valueOf(LerValor.FormatNumber(regFields[1][3].toString(), 2).replace(",", "."));
                } else {
        ...
        return new float[]{fComissao, rComissao};
    }
